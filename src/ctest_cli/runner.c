...
int session_exit(SessionMetrics *session) {
...
int ledger_init(Vector *ledger) {
  return vector_init(ledger, CTEST_MAX_FAIL_LINE_LEN);
}

void ledger_free(Vector *ledger) { vector_free(ledger); }

void test_binaries_free(Vector *test_bin) {
...
int ctest_run_session(const char *dir) {
  ...
  ctest_report_start_banner(dir);

  Vector *test_bins = ctest_discover_tests(dir);
  if (test_bins == NULL) {
    ledger_free(&ledger);
    return -1;
  }
